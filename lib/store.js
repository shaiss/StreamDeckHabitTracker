...
export async function getSlots() {
  ...
  if (!raw) return { slots: [null, null, null, null], suggestedAt: 0, model: '' };
  try {
    const doc = JSON.parse(raw);
    const slots = Array.isArray(doc.slots) ? doc.slots.slice(0, 4) : [];
    while (slots.length < 4) slots.push(null);
    return { slots, suggestedAt: doc.suggestedAt || 0, model: doc.model || '' };
  } catch {
    return { slots: [null, null, null, null], suggestedAt: 0, model: '' };
  }
}

export async function setSlots(doc) {
  return cmd(['SET', SLOTS_KEY, JSON.stringify(doc)]);
}
