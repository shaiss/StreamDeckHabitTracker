...
// Append several entries in one RPUSH — atomic in Redis, so a multi-entry
// NL commit either fully lands or fully fails; a client retry after an error
// can never duplicate a partially-written batch.
export async function appendMany(entries) {
  if (!entries.length) return 0;
  return cmd(['RPUSH', KEY, ...entries.map((e) => JSON.stringify(e))]);
}
