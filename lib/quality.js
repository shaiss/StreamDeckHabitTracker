...
const FIXED = new Set(BASE_HABITS.map((h) => h.name.toLowerCase()));
...
function reasonSpecificity(reason) {
...
export function scoreOutput(items, { parseOk = true } = {}) {
  if (!parseOk) {
  ...
  for (const it of arr) {
    ...
    spec += reasonSpecificity(it?.reason);
  }
  const n = arr.length;
  const out = {
    jsonValid: 1,
