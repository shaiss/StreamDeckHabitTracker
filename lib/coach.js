...
export function summarize(entries, days = 14) {
  ...
  for (const e of recent) {
    ...
    s.taps++;
    s.hours[localHour(e.t)] = (s.hours[localHour(e.t)] || 0) + 1;
    s.days.add(localDay(e.t));
  }
  return {
    timezone: HOME_TZ,
    daysOfData: new Set(recent.map((e) => localDay(e.t))).size,
...
export async function reactTo(entry) {
  ...
  const today = entries.filter((e) => localDay(e.t) === localDay(Date.now()));
  const { text, engine } = await ask(
    ...
    'Context: ' +
    JSON.stringify({
      justTapped: { habit: entry.h, emoji: entry.e || '', viaAiSlot: Boolean(entry.slot), localHour: localHour(entry.t) },
      todayTaps: today.map((e) => ({ h: e.h, hour: localHour(e.t) })),
  ...
  const slots = sanitize(parsed.slots, { defaultTtlMs: REACTIVE_TTL_MS });
...
export async function morningPass() {
  const prev = await getSlots();
  const entries = await all();
  const yesterday = entries.filter((e) => localDay(e.t) === localDay(Date.now() - 86400_000));
  const { text, engine } = await ask(
    `It is morning for your human (${HOME_TZ}). Review yesterday, your notes, and set the keys you want ` +
    'active for the day ahead. You may add "ttlMinutes" (15-720) to time-scoped keys. ' +
    'Reply with ONLY JSON: {"slots":[{"habit":"OneWordId","emoji":"🌅","label":"Short","reason":"..."}]}\n\n' +
    'Context: ' +
    JSON.stringify({
      fixedHabits: BASE_HABITS.map((h) => `${h.emoji} ${h.name}`).join(', '),
      currentAiSlots: prev.slots,
      yesterdayTaps: yesterday.map((e) => ({ h: e.h, hour: localHour(e.t) })),
      last14Days: summarize(entries)
    })
  );
  const slots = sanitize(extractJson(text).slots);
...
export async function dailyDigest() {
  ...
  const { text, engine } = await ask(
    ...
    'hypothesis, and what you are watching for tomorrow. Warm, specific, no filler. ' +
    'Reply with ONLY JSON: {"insight":"..."}\n\n' +
    'Context: ' +
    JSON.stringify({
      todayTaps: today.map((e) => ({ h: e.h, hour: localHour(e.t) })),
      last14Days: summarize(await all())
    })
  );
  const insight = String(extractJson(text).insight || '').slice(0, 600);
  if (!insight) return null;
  const doc = { text: insight, date: localDay(Date.now()), model: zaiModel(), engine, at: Date.now() };
