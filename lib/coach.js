...
import { all, getSlots, setSlots, setInsight, getProfile, getSlotHistory, getRoster, setRoster } from './store.js';
import { chat, extractJson, zaiModel } from './ai.js';
...
export async function fullSuggest() {
  ...
  const { text, engine } = await ask(PROMPTS.suggest(context));
...
export async function morningPass() {
  ...
  const { text, engine } = await ask(PROMPTS.morning(context));
  const rawSlots = extractJson(text).slots;
...
export async function rosterPass() {
  ...
  const { text, engine } = await ask(PROMPTS.roster(context));
  // "No changes today" is the expected answer most days, so an unreadable
  // reply means no proposals — not a failed request. Report it rather than
  // swallowing it, but never let it 500 the caller.
  let raw = null;
  let parseError = '';
  try {
