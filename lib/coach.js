...
function capture(kind, context, rawSlots, engine) {
  captureItem({ kind, context, rawSlots, engine, model: zaiModel() }).catch(() => {});
}
...
export async function fullSuggest() {
  ...
  capture('suggest', context, rawSlots, engine);
  return commitSlots(rawSlots, engine);
}
...
export async function reactTo(entry) {
  ...
  capture('react', context, parsed.slots, engine);
...
export async function morningPass() {
  ...
  const rawSlots = extractJson(text).slots;
  capture('morning', context, rawSlots, engine);
  return commitSlots(rawSlots, engine);
}
