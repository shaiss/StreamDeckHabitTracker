...
// what lands (issue #6).
import { all, getSlots, setSlots, getCoachPage, setCoachPage, COACH_PAGE_SIZE, setInsight, getProfile, getSlotHistory, getRoster, setRoster, getNudgeState, setNudgeState, getDismissals, getQuestions, appendQuestion, patchQuestion } from './store.js';
...
export const COACHPAGE_COOLDOWN_MS = 30_000;
...
// Coach-page pass (#52): the AI-curated "considered layer". The coach assembles
// up to 12 habits it is tracking beyond its 4 front slots, written to
// habits:coach:page, which the deck renders as slots 5..16. Run by the morning
// cron (advisory, never fails the day's keys) and on demand via /api/coachpage.
//
// Reserves the fixed habits AND the current front slots so the page never
// echoes a permanent key or something already on the front page. Like the
// roster pass, it is deliberately NOT captured into the dataset — that corpus
// and lib/quality.js are 4-slot-shaped. A framework hiccup or unusable reply
// returns null (the caller answers 502 / the cron reports zero keys).
export async function coachPagePass() {
  const ctx = await gather();
  const front = (await getSlots()).slots.filter(Boolean).map((s) => s.habit);
  const prevPage = await getCoachPage();
  const reserved = [...ctx.reserved, ...front.map((n) => n.toLowerCase())];
  const context = {
    human: ctx.human,
    fixedHabits: ctx.fixedHabitsStr,
    currentFrontSlots: front.join(', ') || 'none',
    currentCoachPage: prevPage.slots.filter(Boolean).map((s) => s.habit).join(', ') || 'none',
    trackRecord: ctx.trackRecord,
    last14Days: summarize(ctx.entries, ctx.tzh)
  };
  let text, engine;
  try {
    ({ text, engine } = await ask(PROMPTS.coachPage(context)));
  } catch {
    return null;
  }
  const parsed = extractJson(text);
  const slots = sanitize(parsed?.slots, { max: COACH_PAGE_SIZE, reserved });
  if (!slots.length) return null;
  while (slots.length < COACH_PAGE_SIZE) slots.push(null);
  const doc = { slots, updatedAt: Date.now(), model: zaiModel(), engine };
  await setCoachPage(doc);
  return doc;
}
